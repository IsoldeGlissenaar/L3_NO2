...
fig, axs = plt.subplots(3, 2, figsize=(7.5, 5), dpi=400, constrained_layout=True, subplot_kw={'projection': proj})

#Superobservation - weighted
axs[0,0].coastlines(resolution='50m',linewidth=0.3)
im=axs[0,0].pcolormesh(ds.longitude,ds.latitude,ds.tropospheric_NO2_column_number_density/1e15,
...
im=axs[1,0].pcolormesh(ds.longitude,ds.latitude,ds.tropospheric_NO2_column_number_density_temporal_std/1e15,
                vmin=0,vmax=1,cmap='YlOrRd',transform=ccrs.PlateCarree())
axs[1,0].text(0.01,0.92,'(b)',fontsize=8,transform=axs[1,0].transAxes)

#STD2
axs[2,0].coastlines(resolution='50m',linewidth=0.3)
im=axs[2,0].pcolormesh(ds.longitude,ds.latitude,ds.tropospheric_NO2_column_number_density_uncertainty_kernel/1e15,
                vmin=0,vmax=1,cmap='YlOrRd',transform=ccrs.PlateCarree())
...
im=axs[0,1].pcolormesh(ds.longitude,ds.latitude,ds.tropospheric_NO2_column_number_density/1e15,
               vmin=0,vmax=10,cmap='Spectral_r',transform=ccrs.PlateCarree())
...
im=axs[1,1].pcolormesh(ds.longitude,ds.latitude,ds.tropospheric_NO2_column_number_density_temporal_std/1e15,
                vmin=0,vmax=1,cmap='YlOrRd',transform=ccrs.PlateCarree())
...
im=axs[2,1].pcolormesh(ds.longitude,ds.latitude,ds.tropospheric_NO2_column_number_density_uncertainty_kernel/1e15,
                vmin=0,vmax=1,cmap='YlOrRd',transform=ccrs.PlateCarree())
...
ams = ds.tropospheric_NO2_column_number_density_uncertainty_kernel.values[711,924]/1e15
...
beij = ds.tropospheric_NO2_column_number_density_uncertainty_kernel.values[649,1481]/1e15
print(f'Beijing std2: {np.round(beij,2)} 1e15 molecules/cm2')
print('--')
afr = np.nanmax(ds.tropospheric_NO2_column_number_density.values[462:515,797:1093])/1e15
print(f'Africa mean: {np.round(afr,2)} 1e15 molecules/cm2')
afr = np.nanmax(ds.tropospheric_NO2_column_number_density_temporal_std.values[462:515,797:1093])/1e15
print(f'Africa std1: {np.round(afr,2)} 1e15 molecules/cm2')
afr = np.nanmax(ds.tropospheric_NO2_column_number_density_uncertainty_kernel.values[462:515,797:1093])/1e15
...
print(f'Beijing std2: {np.round(beij,2)} 1e15 molecules/cm2')
print('--')
afr = np.nanmax(ds.tropospheric_NO2_column_number_density.values[384:437,952:1093])/1e15
print(f'Africa mean: {np.round(afr,2)} 1e15 molecules/cm2')
afr = np.nanmax(ds.tropospheric_NO2_column_number_density_temporal_std.values[384:437,952:1093])/1e15
print(f'Africa std1: {np.round(afr,2)} 1e15 molecules/cm2')
afr = np.nanmax(ds.tropospheric_NO2_column_number_density_uncertainty_kernel.values[384:437,952:1093])/1e15
print(f'Africa std2: {np.round(afr,2)} 1e15 molecules/cm2')
